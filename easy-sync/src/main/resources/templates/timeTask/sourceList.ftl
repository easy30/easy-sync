...
<script>
    var editPage="sourceEdit.htm";
    var taskType="${taskType}";
    searchForm.action="sourceList.htm";
...
<div class="container-fluid">
    ...
    <table class="table table-striped table-hover" style="word-break: break-all; word-wrap: break-word;">
        <tr>
           ...
           <#-- <th class="canHide" lang-key="plan">plan</th>-->
        ...
        <#list list as item>
            <tr>
                ...
                <td>${item.id}  <#if  item.priority==2 > <span class="label label-danger" lang-key="high">高</span></#if></td>
                <td>${item.name}</td>
                ...
                <td><a href="#" class="btn btn-primary btn-xs" onclick="edit(${item.id})" lang-key="modify">modify</a>
                    ...
                    <#if areaCustom.taskConfig??>
                        <a href="${ctx}/search/index.htm?admin=${admin}&timeTaskId=${item.id}&errorFlag=-1"
                           class="btn btn-info btn-xs" target="_blank" lang-key="browseData"></a>
                    </#if>
                    <span style="width:20px">&nbsp;</span>

                    <a href="#" class="btn btn-primary btn-xs" onclick="doPost('copy.htm?id=${item.id}',1)"  lang-key="copy"></a>
